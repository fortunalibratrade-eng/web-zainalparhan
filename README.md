...
Tombol **Ekspor Excel (.xlsx)** di tab **Pengaturan → Cadangkan Data** menghasilkan satu file Excel siap-pakai berisi 4 sheet yang otomatis sinkron dengan data yang sudah dimasukkan (bisa diklik kapan saja, tanpa perlu login):
1. **Ringkasan** — identitas KKL, periode, dan rekap keuangan & proker.
2. **Anggota** — daftar anggota lengkap dengan status iuran (Lunas/Sebagian/Belum Bayar, sudah memperhitungkan batas maksimal Rp700.000).
3. **Transaksi** — seluruh transaksi terurut tanggal, warna hijau/merah untuk pemasukan/pengeluaran, plus baris total saldo kas.
4. **Program Kerja (RAD)** — matriks lengkap sesuai format Rencana Aksi Detail (kolom Tujuan Spesifik, Sasaran/Peserta, Metode, Waktu & Tempat, Penanggung Jawab, Kebutuhan Sumber Daya, Indikator Keberhasilan, Anggaran, Realisasi, Status).

File Excel sudah diformat rapi (header berwarna, garis tabel, format Rupiah, baris zebra, freeze header, filter otomatis) sehingga langsung layak dipakai untuk laporan resmi.
